...
export default function DailyTodoPanel({ selectedDate }: Props) {
  ...
  const removeContentRow = (idx: number) => setAddContents((prev) => prev.filter((_, i) => i !== idx))
  const updateContent = (idx: number, val: string) => setAddContents((prev) => prev.map((c, i) => (i === idx ? val : c)))
          ...
          <ul style={{ margin: 0, padding: 0, listStyle: 'none', display: 'grid', gap: 6 }}>
            {todos.length === 0 ? (
              ...
              todos.map((todo) => (
                <li
                  key={todo.id}
                  style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: 8, border: '1px solid #eceef2', borderRadius: 10, background: '#fff', padding: 8, transition: 'border-color .15s ease', ...(todo.is_checked && { borderColor: '#ffbe8f', boxShadow: '0 0 0 1px #ffd7bc inset' }) }}
                >
                  <span style={{ display: 'flex', alignItems: 'center', gap: 7, minWidth: 0 }}>
                    ...
                    {todo.planned_time && (
                      <span style={{ fontSize: 12, color: '#a05a26', fontWeight: 700, minWidth: 42, flexShrink: 0 }}>
                        {todo.planned_time}
                      </span>
                    )}
