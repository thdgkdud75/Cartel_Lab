    ...
    <section style={{ border: '1px solid #eceef2', borderRadius: 14, background: '#fff', marginBottom: 14, padding: 12, position: 'relative', overflow: 'visible' }}>
        ...
        <div style={{ position: 'absolute', left: 12, right: 12, top: 56, zIndex: 35, border: '1px solid #ffd2b3', borderRadius: 12, background: '#fff', boxShadow: '0 12px 28px rgba(0,0,0,0.12)', padding: 12 }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 8 }}>
            ...
            <button
              ...
              style={{ border: 0, background: 'transparent', color: '#8a9099', fontSize: 20, lineHeight: 1, cursor: 'pointer' }}
            >
              ×
            </button>
          </div>
      ...
      <ul style={{ margin: 0, padding: 0, listStyle: 'none', display: 'grid', gap: 6 }}>
        {goals.length === 0 ? (
          ...
          goals.map((goal) => (
            <li key={goal.id} style={{ padding: 10, border: '1px solid #f2e3d7', background: '#fffaf5', borderRadius: 10 }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: 10 }}>
                ...
                {session && (
                  <button
                    ...
                    style={{ border: '1px solid #ffd2b3', borderRadius: 8, background: '#fff', color: '#9f3f10', fontSize: 12, fontWeight: 700, padding: '6px 10px', cursor: 'pointer' }}
                  >
                    삭제
                  </button>
                )}
